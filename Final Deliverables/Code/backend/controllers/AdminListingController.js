...
async function updateSimilarProducts(newProduct) {
    return new Promise((resolve, reject) => {
        const python = spawn("python3", ["SimilarProducts/update_similar_products.py"]);

        let output = "";
        let errorOutput = "";

        // Send product to Python
        python.stdin.write(JSON.stringify(newProduct));
        python.stdin.end();
        ...
        python.on("close", (code) => {
            if (code !== 0) {
                ...
                return reject("Similarity update failed.");
            }
    ...
    try {
        ...
        if (newStatus === "Approved") {
            ...
            for (const product of approvedProducts) {
                ...
                try {
                    await updateSimilarProducts({
                        _id: product._id.toString(),
                        title: product.title,
                        description: product.description
                    });
                    console.log(`SimilarProducts updated for ${product._id}`);
                } catch (err) {
                    console.error(`SimilarProducts update failed for ${product._id}:`, err);
                }
