...
exports.fetchSimilarProducts = async (req, res) => {
  try {
    ...
    const objectId = new mongoose.Types.ObjectId(id);
    const similarProducts = await SimilarProducts.find();

    const similarEntry = await SimilarProducts.findOne({ productId: objectId });

    if (!similarEntry || !similarEntry.similarProducts || similarEntry.similarProducts.length === 0) {
      return res.status(404).json({ message: "No similar products found." });
    }

    // Sort similarProducts by similarity score in descending order
    similarEntry.similarProducts.sort((a, b) => b.similarityScore - a.similarityScore);

    // Extract top similar product IDs (excluding self if included)
    const similarProductIds = similarEntry.similarProducts
      .filter(item => item.productId.toString() !== id)
      .map(item => item.productId);

    // Fetch products and sort them by their corresponding similarity score
    ...
    // Map products back to include their similarity scores for sorting
    const sortedProducts = products.map(product => {
      const similarityScore = similarEntry.similarProducts.find(item => item.productId.toString() === product._id.toString()).similarityScore;
      return { product, similarityScore };
    })
    .sort((a, b) => b.similarityScore - a.similarityScore)  // Sort by similarity score

    // Send the sorted products along with similarity scores
    return res.status(200).json({ similarProducts: sortedProducts.map(item => item.product) });
  } catch (error) {
    console.error("Error fetching similar products:", error);
    return res.status(500).json({ message: "Server error fetching similar products." });
  }
};
